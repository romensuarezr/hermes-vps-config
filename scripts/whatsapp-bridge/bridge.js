...
const WHATSAPP_GROUP_POLICY = String(process.env.WHATSAPP_GROUP_POLICY || 'open').trim().toLowerCase();
...
if (PAIR_ONLY) {
  ...
  app.listen(PORT, '127.0.0.1', () => {
    ...
    if (ALLOWED_GROUPS.size > 0) {
      console.log(`🔒 Allowed groups: ${Array.from(ALLOWED_GROUPS).join(', ')}`);
    } else if (WHATSAPP_GROUP_POLICY === 'open') {
      console.log(`🔓 WHATSAPP_GROUP_POLICY=open — all groups are accepted.`);
    } else {
      console.log(`🔒 No WHATSAPP_GROUP_ALLOWED_USERS set — all groups are rejected.`);
    }
